    ...
    <Card id={product.id} className="flex flex-col overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 h-full">
            ...
            <div className="flex flex-wrap gap-2">
              {product.subscriptionOptions.map(option => (
                <Badge key={option.type} variant="secondary" className="font-normal">
                  {option.type === 'monthly' ? t({ en: "Monthly", ta: "மாதாந்திரம்" }) : t({ en: "Yearly", ta: "வருடாந்திரம்" })}: ${option.price.toFixed(2)}
                  {option.type === 'yearly' && <span className="ml-1 text-xs opacity-80">/yr</span>}
                  {option.type === 'monthly' && <span className="ml-1 text-xs opacity-80">/mo</span>}
                </Badge>
              ))}
            </div>
          </div>
      ...
      <CardFooter>
        <Button variant="outline" className="w-full" asChild>
          <Link href={`/contact?product=${product.id}`}>{t({ en: "Learn More / Request Demo", ta: "மேலும் அறிக / டெமோ கோரிக்கை" })}</Link>
        </Button>
      </CardFooter>
    </Card>
  );
